import gym
import numpy as np
import ComputeSamples
import hdirllib.rlagents.ReinforceA2C as reinfA2C
import hdirllib.simulators.pythonSimulators as pySim
import torch
import time
...
def step(states, actions, log_probs, rewards, returnLogProbs, expandActionDim, returnRewards, trajId, pol, gymStep, state):
    policyRes = pol(state)
    if (returnLogProbs):
    ...
    state, reward, done, _ = gymStep(action)
    ...
    return done, states, actions, log_probs, rewards
...
class GymSimulator1:
    ...
    def SampleTrajectoriesFromS0(self, s0, policy, returnRewards=False, returnLogProbs=False, maxIterations=400):
        """ Samples Ntrajs rajectories from given state(s) s0
                ...
                returnRewards -- whether the method should return ONLY rewards or not
                returnLogProbs -- whether the method should return log_probs together with actions
        ...
        for trajId in np.arange(s0.shape[0]):
            ...
            test = ComputeSamples.ComputeSamples(step, maxIterations, states, actions, log_probs, rewards, returnLogProbs, expandActionDim, returnRewards, trajId, policy, self.gymInstance.step, state)
            states, actions, log_probs, rewards, trajId = test.returnResult()
    ...
    def SampleTrajectoriesFromStateSamplerParallel(self, pool,stateSampler, policy, Ns0=1, maxIterations=400):
        """ Samples Ns0 trajectories from given stateSampler
                ...
                policy -- a function state --> action, may include sampler from distribution
                returnRewards -- whether the method should return ONLY rewards or not
                returns [Ns0,NNested,stateSpaceShape,time(list dimension?)], [Ns0,NNested,actionSpaceShape,time(list dimension?)], [Ns0,NNested,1,time(list dimension?)], [Ns0]
                this is states, actions, rewards, trajectoryLengths
                or (if returnRewards)
...
class GymTensorStateSimulator:
    ...
    def SampleTrajectoriesFromStateSampler(self, stateSampler, policy, Ns0=1,\
        ...
        """ Samples Ns0 trajectories from stateSampler
                ...
                returnRewards -- whether the method should return ONLY rewards or not
                returnLogProbs -- whether the method should return log_probs together with actions
                stateMemorySize -- amount of additional frames representing a single state
                grayscale -- convert multilayered images to gray image
                downsample -- lower the resolution by picking every second pixel
                returns [Ns0,stateSpaceShape,time],[Ns0,actionSpaceShape,time-1],[Ns0,1,time-1], [Ns0]
                    this is states, actions, rewards, trajectoryLengths; 
                    time is a variable dimension: length of the longest trajectory
                or (if returnRewards)
